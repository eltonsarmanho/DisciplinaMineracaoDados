...
\begin{enumerate}
  ...
  \item \textbf{Reconhecimento de emoções:} classifica texto em emoções básicas -- alegria, tristeza, raiva, medo, desgosto, surpresa (Ekman, 1992).
...
\draw[arr] (feat)--(model);
\draw[arr] (model)--(out);
\node[font=\tiny,below=0.15cm] at (3,0) {tokenize, clean,\\normalize};
\node[font=\tiny,below=0.15cm] at (6,0) {TF-IDF / Embeddings\\/ BERT tokens};
\node[font=\tiny,below=0.15cm] at (9,0) {VADER / SVM\\/ BERT fine-tuned};
\end{tikzpicture}
\end{center}
