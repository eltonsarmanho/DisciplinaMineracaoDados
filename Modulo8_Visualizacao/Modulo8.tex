...
\begin{enumerate}[label=\faIcon{check-circle}~\textbf{\arabic*.}, leftmargin=2cm, itemsep=0.3em]
  \item Criar mais de 12 tipos de gráficos estatísticos (distribuição, relação, comparação e composição) com Matplotlib e Seaborn.
  \item Aplicar técnicas de redução de dimensionalidade (PCA, t-SNE e UMAP) para visualização de dados de alta dimensão.
  \item Construir visualizações interativas e animadas com Plotly (séries temporais, bubble charts animados, sunburst).
  \item Visualizar e interpretar resultados de modelos de ML: curvas ROC, Precision-Recall, matrizes de confusão, importância de atributos e gráficos SHAP.
  \item Visualizar resultados de clustering com silhouette plot, curva do cotovelo, t-SNE e radar charts de centroides.
  \item Visualizar regras de associação com grafos direcionados, heatmaps de co-ocorrência e bubble charts suporte vs.\ confiança.
  \item Gerar relatórios automáticos de EDA com \texttt{ydata-profiling}.
  \item Construir um dashboard analítico completo e exportável (HTML) com Plotly Subplots.
\end{enumerate}
...
\begin{enumerate}[label=\textbf{\arabic*.}, leftmargin=1.5cm, itemsep=0.2em]
  \item Quarteto de Anscombe --- Por que Visualizar?
  \item Gráficos Estatísticos Fundamentais
  \item Visualização Interativa com Plotly
  \item Visualização de Resultados de ML
  \item Visualização de Clustering
  \item Visualização de Regras de Associação
  \item Auto-EDA com \texttt{ydata-profiling}
  \item Mini-Dashboard com Plotly Subplots
  \item Exercícios
\end{enumerate}
...
\begin{dicabox}{Implementação no Colab}
Veja o notebook do Módulo 8 para implementação completa com Matplotlib e Seaborn: histograma + KDE, boxplot por grupo e violin plot com dataset California Housing.
\end{dicabox}
...
\begin{dicabox}{Implementação no Colab}
O notebook do Módulo 8 implementa PCA, t-SNE e UMAP com dataset Iris normalizado, gerando projeções 2D coloridas por espécie para comparação visual das três técnicas.
\end{dicabox}
...
\begin{dicabox}{Implementação no Colab}
O notebook do Módulo 8 constrói um painel \texttt{make\_subplots} 2$\times$2 com curva ROC, matriz de confusão, importância de atributos e curvas de aprendizado, tudo interativo no browser.
\end{dicabox}
...
Dash (Plotly) é um framework para construção de aplicações analíticas web em Python puro. Seus principais componentes são: \texttt{dcc.Graph} para gráficos interativos, \texttt{dcc.Dropdown/Slider} para controles de entrada e \texttt{@app.callback} para atualização reativa de gráficos em resposta a interações do usuário.

\subsection{Streamlit}

Streamlit permite criar dashboards analíticos em minutos, com suporte a abas (\texttt{st.tabs}), sliders laterais (\texttt{st.sidebar}), gráficos Plotly e tabelas interativas --- tudo sem HTML ou JavaScript.

\begin{dicabox}{Implementação no Colab}
O notebook do Módulo 8 inclui exemplos de Dash com callbacks e Streamlit com slider de clusters. Execute localmente ou via \texttt{ngrok} para expor o servidor no Colab.
\end{dicabox}
...
\begin{dicabox}{Implementação no Colab}
O notebook do Módulo 8 aplica SHAP com XGBoost no dataset Cancer de Mama, gerando beeswarm summary plot, bar summary e waterfall plot para exemplos individuais.
\end{dicabox}
...
\node[text=white,font=\bfseries] at (8,9.5) {Dashboard: Mineração de Dados};
...
\node[font=\small,minegray] at (4,6.2) {Importância de Atributos};
% Bar chart sketch
