...
\def\radius{3.2}
\def\innerradius{0.8}
% Setores 0-4: loop simples com \colorlet para evitar espaço na expansão de \col
\foreach \idx/\sectxt/\cname in {%
  ...
  \node[font=\tiny\bfseries,align=center,text width=1.8cm] at (\secMid:2.1) {\sectxt};%
}%
% Setor 5 separado: cor composta minegreen!60!mineblue (não suportada via variável foreach)
...
\fill[tmpsec!30] (\secA:\innerradius) -- (\secA:\radius) arc(\secA:\secB:\radius) -- (\secB:\innerradius) arc(\secB:\secA:\innerradius) -- cycle;%
\draw[tmpsec,thick] (\secA:\innerradius) -- (\secA:\radius) arc(\secA:\secB:\radius) -- (\secB:\innerradius) arc(\secB:\secA:\innerradius) -- cycle;%
\node[font=\tiny\bfseries,align=center,text width=1.8cm] at (\secMid:2.1) {Implanta\c{c}\~{a}o};
...
\midrule
Detecção de Anomalias & Semi-Supervisionado & Identificar instâncias atípicas & Fraude financeira; falha em equipamentos & Isolation Forest, LOF, One-Class SVM \\
\midrule
