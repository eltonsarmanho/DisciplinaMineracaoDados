...
\begin{frame}{Transformers e Mecanismo de Atenção}
    \begin{columns}
        \column{0.5\textwidth}
        \begin{block}{Atenção Escalada (Vaswani et al., 2017)}
            \begin{equation*}
                \text{Attention}(Q,K,V) = \text{softmax}\!\left(\frac{QK^\top}{\sqrt{d_k}}\right)V
            \end{equation*}
            Permite que cada posição ``atenda'' a todas as outras
        \end{block}
        \begin{block}{BERT}
            Embeddings de texto de alta qualidade via pré-treino bidirecional
        \end{block}
        \column{0.5\textwidth}
        \begin{block}{TabNet (Arik \& Pfister, 2021)}
            \begin{itemize}
                \item Transformer adaptado para \textbf{dados tabulares}
                \item Seleção de features \textbf{interpretável} passo a passo
                \item Atenção esparsa: foca nas features mais relevantes
                \item Competitivo com GBDTs em benchmarks
            \end{itemize}
        \end{block}
        \vspace{0.2cm}
        \tiny \textit{Ver Material Didático Módulo 5 para detalhes matemáticos}
    \end{columns}
\end{frame}
...
\begin{frame}[shrink=5]{HPO: Otimização de Hiperparâmetros}
    \begin{columns}
        \column{0.5\textwidth}
        \begin{block}{Grid Search}
            $\mathcal{O}(n^k)$, método exaustivo e impraticável em alta dimensão\\
            \small Testa todas as combinações possíveis
        \end{block}
        \begin{block}{Random Search (Bergstra \& Bengio, 2012)}
            Melhor cobertura do espaço de busca\\
            \small Encontra boas soluções com menos avaliações
        \end{block}
        \begin{block}{Otimização Bayesiana (TPE)}
            Usa modelo \textit{surrogate} para balancear\\
            \textbf{exploração} $\times$ \textbf{explotação}\\
            \small Tree-structured Parzen Estimator
        \end{block}
        \column{0.5\textwidth}
        \begin{block}{Optuna (Akiba et al., 2019)}
            Framework Python com:
            \begin{itemize}
                \item \faCheck\ Sampler TPE por padrão
                \item \faCheck\ Pruning de trials não promissoras
                \item \faCheck\ Dashboard visual interativo
                \item \faCheck\ Suporte a otimização paralela
                \item \faCheck\ API simples e flexível
            \end{itemize}
        \end{block}
        \vspace{0.2cm}
        \centering\tiny \textit{\faArrowRight\ Consulte o Material Didático e Colab para experimentos}
    \end{columns}
\end{frame}
