...
\begin{center}
\begin{tikzpicture}
  % Dimensões fixas — estrutura independente do texto
  \def\cw{3.2}   % largura de cada coluna (cm)
  \def\hh{0.85}  % altura da linha de cabeçalho
  \def\ch{1.6}   % altura das linhas de dados

  % ── Preenchimento dos retângulos ──────────────────────────
  % Cabeçalho
...
\end{tikzpicture}
\captionof{figure}{Matriz de confusão para classificação binária. Diagonal principal (VP, VN): predições corretas. Fora da diagonal (FN, FP): erros de classificação.}
\end{center}
