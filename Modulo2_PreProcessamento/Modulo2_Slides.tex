...
\begin{frame}{Escalas de Medida de Stevens (1946)}
    ...
    \begin{columns}[T]
        ...
        \begin{exampleblock}{Estruturados vs.\ Não Estruturados}
            \small Dados tabulares (SQL) vs.\ texto, imagem, áudio — requerem extração de features.
        \end{exampleblock}

        \column{0.5\textwidth}
        \begin{exampleblock}{Séries Temporais e Espaciais}
            \small Sequências ordenadas no tempo ou com componente geográfico — técnicas especiais necessárias.
        \end{exampleblock}
    \end{columns}
\end{frame}
...
\begin{frame}{O que é um Outlier? --- Método IQR}
    \begin{columns}[T]
        \column{0.4\textwidth}
