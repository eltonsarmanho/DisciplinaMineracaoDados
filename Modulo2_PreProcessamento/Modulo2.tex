...
\begin{tikzpicture}[
  ...
  % ── Títulos (centralizados em cada grade) ─────────────────────
  % Grade 6×5, célula 0.5cm: centro da col 1 = 0.25, col 6 = 2.75 → centro grade = 1.5
  % Offset MAR = +3.5, Offset MNAR = +7.0
  ...
  % ── Grade MCAR: ausências completamente aleatórias ────────────
  \foreach \c in {1,...,6}{
    \foreach \r in {1,...,5}{
      \node[cell,fill=mineblue!15] at ({(\c-0.5)*0.5},{(\r-0.5)*0.5}) {};
    }
  }
  % 6 células ausentes em posições aleatórias dentro da grade MCAR
  ...
  % ── Grade MAR: ausências dependem de variável observada ───────
  \foreach \c in {1,...,6}{
    \foreach \r in {1,...,5}{
      \node[cell,fill=mineorange!15] at ({(\c-0.5)*0.5+3.5},{(\r-0.5)*0.5}) {};
    }
  }
  % Cols 5-6 (x=5.75, 6.25), linhas 3-5 (y=1.25, 1.75, 2.25) ausentes
  ...
  % ── Legenda centralizada sob as três grades ───────────────────
...
Em um dataset de transações com 0.1\% de fraudes: um modelo que sempre prevê ``não é fraude'' tem 99.9\% de acurácia mas é completamente inútil. As métricas adequadas para dados desbalanceados são Precision, Recall, F1-Score e AUC-ROC.
\end{exemplobox}
